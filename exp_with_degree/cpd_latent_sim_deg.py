...
def parse_args():
  ...
  parser.add_argument('--num_time', default=90) # UPDATED
  parser.add_argument('--true_CP_half', default=[13,24,35]) # UPDATED
  parser.add_argument('--true_CP_full', default=[24,46,69]) # UPDATED
...
def learn_one_seq_penalty(args, adj_train, adj_test, pen_iter, seq_iter, half):
  ...
  for learn_iter in range(args.epoch):
    ...
    ############################
    # only during testing with full data
    if not half:
    ...
    if (learn_iter+1) == args.epoch or early_stopping:
      ...
      with torch.no_grad():
        if half:
          ...
          return torch.tensor(result[0:5]), loglik, mu, adj_prob
        else:
          # USE THE BEST MU
          ...
          return torch.tensor(result[0:5]), best_loglik, best_mu, best_adj_prob
...
for seq_iter in range(0,args.num_seq):
  ...
  identity_matrices = torch.eye(args.num_node).unsqueeze(0).repeat(one_seq.shape[0], 1, 1)
  one_seq = one_seq + identity_matrices # add self edges

  keep_indices = [i for i in range(100) if (i + 1) % 10 != 0]
  one_seq = one_seq[keep_indices,:,:] # 90 by n by n
  ...
  output_holder[seq_iter,:], _, mu_seq_output, adj_prob_seq_output = \
  ...
  np.save(os.path.join(output_dir,'adj_prob_seq{}.npy'.format(seq_iter)), adj_prob_seq_output.cpu().numpy())
